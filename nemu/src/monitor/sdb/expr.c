...
// };
enum {
  TK_NOTYPE = 256, TK_EQ,TK_NEQ, TK_OR, TK_AND, TK_NUM, TK_REG, TK_REF, TK_NEG
};
...
void init_regex() {
  ...
  for (i = 0; i < NR_REGEX; i ++) {
    ...
    // else{
    //   ret = regexec(&re[i], buf, nmatch, pmatch, 0);
    //   if(ret != 0){
    //     regerror(ret, &re[i], error_msg, 128);
    //     panic("regex match failed: %s\n%s", error_msg, rules[i].regex);
    //   }
    //   else{
...
char e_str[32];
char* e_value(char *e,int position,int substr_len){
    //char *str=NULL;
    int i=0;
    for (i = position;(i<substr_len+position);i++){
      e_str[i-position]=e[i];
    }
    //str[i-position]='\0';
    return e_str;
}
...
//                      memcpy(tokens[nr_token].str,e_value(e,position-substr_len,substr_len),32);
//                      printf("%d is NUM   %s   \n",nr_token,tokens[nr_token].str);
//                      nr_token+=1;break;

                     
//           case '+': tokens[nr_token].type =  rules[i].token_type;printf("%d is %c\n",nr_token,tokens[nr_token].type);
//                      nr_token+=1;break;
//           case '-': tokens[nr_token].type =  rules[i].token_type;printf("%d is %c\n",nr_token,tokens[nr_token].type);
//                      nr_token+=1;break;
//           case '=': tokens[nr_token].type =  rules[i].token_type;printf("%d is  %c\n",nr_token,tokens[nr_token].type);
...
//       //   Log("m1atch rules[%d] = \"%s\" at position %d with len %d: %.*s",
//       //       i, rules[i].regex, position, substr_len, substr_len, substr_start);
...
//       //   // switch (rules[i].token_type) {
//       //   //   case '+': tokens[nr_token].type =  rules[i].token_type; nr_token+=1;break;
//       //   //   default: ;
//       //   // }
  ...
  if (s > e) {
    ...
    *success = true;
    ...
    if (op_type == '!' || op_type == TK_NEG || op_type == TK_REF) {
      word_t val = eval(dominated_op + 1, e, success);
      if (!*success) { return 0; }
    ...
    word_t val1 = eval(s, dominated_op - 1, success);
    if (!*success) { return 0; }
    word_t val2 = eval(dominated_op + 1, e, success);
    if (!*success) { return 0; }
